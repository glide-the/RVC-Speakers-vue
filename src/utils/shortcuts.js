...
export const calendarBaseShortcuts = [{
  text: '今天',
  onClick(picker) {
    const start = new Date().strftime('%F', 'zh') + ' 00:00:00'
    const end = new Date().strftime('%F', 'zh') + ' 23:59:59'
    picker.$emit('pick', [parseDate(start), parseDate(end)])
  }
}, {
  text: '昨天',
  onClick(picker) {
    const start = new Date().daysAgo(1).strftime('%F', 'zh') + ' 00:00:00'
    const end = new Date().daysAgo(1).strftime('%F', 'zh') + ' 23:59:59'
    picker.$emit('pick', [parseDate(start), parseDate(end)])
  }
}, {
  text: '最近一周',
