...
export function handleTree(data, id, parentId, children, rootId) {
  ...
  const treeData = cloneData.filter(father => {
    ...
    // 返回第一层
    // console.log(typeof father[parentId] , father[parentId])
    let fatherId
    if (isNumberStr(father[parentId])) {
      fatherId = Number(father[parentId])
    } else {
      fatherId = father[parentId]
    }
    return fatherId === rootId
  })
 ...
 /**
     * 注：函数参数中 id 和 parentId是判断某一条数据是数据源中哪条数据的子节点数据的关键
     * （当数据A的parentId等于数据B的id值时，则说明数据A是数据B的子节点数据）
     * @param simpleData 数据源
     * @param id
     * @param parentId
     * @returns {[]}   返回值为一个树形结构数据
     */
  export function toTree(simpleData,id,parentId) {
    let newData = [];
    let map = {};
    for (let i = 0; i < simpleData.length; i++) {
        map[simpleData[i][id]] = simpleData[i];
    }
    for (let i = 0; i < simpleData.length; i++) {
        let element = simpleData[i];
        let parent = map[simpleData[i][parentId]];
        if (parent) {
            (parent.children || (parent.children = [])).push(simpleData[i]);
        }else {
...
// 查询树结构下的信息
export function findTreeDeptChild(deptOptions, findId) {
  let data = null
  for (const dept of deptOptions) {
    if (Number(dept.id) === Number(findId)) {
      data = dept
      break
    }
  }
  if (data == null) {
    for (const dept of deptOptions) {
      if (dept.children) {
        data = findTreeDeptChild(dept.children, findId)
      }
    }
  }
